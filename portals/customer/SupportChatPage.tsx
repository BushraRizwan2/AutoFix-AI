...
export const SupportChatPage: React.FC<SupportChatPageProps> = ({ tickets, selectedTicket, onSelectTicket, onSendMessage, onBack }) => {
        ...
        return (
            <div className="animate-in fade-in duration-300">
                ...
                <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-8">
                    <div>
                        <h1 className="text-4xl font-bold text-slate-900 dark:text-white">Support Ticket</h1>
                        <p className="mt-2 text-lg text-slate-600 dark:text-slate-400">
                           {selectedTicket.subject}
                        </p>
                    </div>
                </div>
                <ChatInterface messages={selectedTicket.chatHistory} onSendMessage={onSendMessage} currentUserSender="customer" />
            </div>
        )
    }
        ...
        <div className="animate-in fade-in duration-300">
            ...
            <div className="bg-white dark:bg-slate-900/50 backdrop-blur-xl rounded-2xl shadow-lg dark:shadow-2xl border border-slate-200 dark:border-slate-700/50 p-6 space-y-4">
                {tickets.map(ticket => (
                 ...
                 {tickets.length === 0 && (
                        <p className="text-center text-slate-500 py-8">You have no support tickets.</p>
                 )}
            </div>
        </div>
    );
